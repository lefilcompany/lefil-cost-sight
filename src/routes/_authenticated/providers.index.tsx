...
import { provisionConnection } from "@/lib/sync.functions";
...
const CONNECTION_SCHEMAS: Record<string, ConnectionSchema> = {
  ...
  Gemini: {
    apiKeyLabel: "API Key do AI Studio ou Service Account JSON",
    apiKeyPlaceholder: 'AIza...  ou  {\n  "type": "service_account",\n  ...\n}',
    apiKeyType: "textarea",
    apiKeyHelper:
      "Cole a API Key (AIza...) para uso por tokens, ou o JSON completo da service account para importar o custo oficial via BigQuery Billing Export.",
    configFields: [
      {
        key: "gcp.bq_project",
        label: "BigQuery Project ID (billing)",
        placeholder: "meu-projeto-billing",
        helper: "Projeto que hospeda o dataset do billing export. Obrigatório no modo BigQuery.",
      },
      {
        key: "gcp.bq_dataset",
        label: "BigQuery Dataset",
        placeholder: "billing_export",
        helper: "Dataset onde o Cloud Billing grava o export.",
      },
      {
      ...
      },
      {
        ...
        helper: "Usado como rótulo no financeiro quando não há eventos nem billing export.",
      },
      {
        key: "plan_monthly_usd",
        label: "Valor mensal fixo (USD, opcional)",
        placeholder: "0",
        helper: "Fallback: registra custo fixo mensal quando não há uso importado.",
        defaultValue: "0",
      },
    ],
    docsUrl: "https://cloud.google.com/billing/docs/how-to/export-data-bigquery",
    setupSteps: [
      "Custo oficial (recomendado): habilite o BigQuery Billing Export (Standard usage cost) na conta de faturamento do projeto que hospeda a chave Gemini.",
      "Crie uma service account com BigQuery Data Viewer no dataset e BigQuery Job User no projeto; baixe a chave JSON e cole no campo acima.",
      "Preencha BigQuery Project ID, Dataset e Billing Account ID. A importação filtra os serviços Generative Language / Vertex AI e grava em cost_entries.",
      "Alternativa simples: cole a API Key do AI Studio (AIza...) — valida o acesso e agrega o uso registrado em gemini_usage_events.",
    ],
  },
